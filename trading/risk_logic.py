from trading.risk_constants import (
    MAX_TRADES_PER_DAY,
    MAX_DAILY_LOSS_PERCENT,
    MAX_OPEN_POSITIONS,
    MIN_TIME_BETWEEN_TRADES_MINUTES
)

import db_logging
import trading.risk_functions as risk_functions
import trading.trade_functions as trade_functions


# Return True if SHOULD buy, else False
def should_buy(symbol, account):

    account_value = risk_functions.get_account_value(account)

    clock = trade_functions.get_market_clock()

    if (
        risk_functions.get_num_trades_today() < MAX_TRADES_PER_DAY
        and risk_functions.get_buying_power(account)
        >= risk_functions.calculate_position_size(account)
        and trade_functions.market_is_open(clock)
        and risk_functions.get_time_between_last_trade(symbol)
        >= MIN_TIME_BETWEEN_TRADES_MINUTES
        and risk_functions.get_daily_pnl_percent(account_value)
        >= MAX_DAILY_LOSS_PERCENT
        and not risk_functions.own_stock(symbol)
        and risk_functions.get_num_open_positions()
        < MAX_OPEN_POSITIONS
    ):
        return True

    return False


# return True if SHOULD sell, else False
def should_sell(symbol):

    # get market clock
    clock = trade_functions.get_market_clock()

    # market closed -> don't sell
    if not trade_functions.market_is_open(clock):
        return False

    # if no position exists -> don't sell
    position = db_logging.get_position(symbol)

    if position is None:
        return False

    # get current stock price
    current_price = risk_functions.get_latest_price(symbol)

    if current_price is None:
        return False

    # get stop levels
    stop_loss = risk_functions.get_stop_loss(symbol)
    trailing_stop = risk_functions.get_trailing_stop(symbol)

    # can't make sell decision without stop values
    if stop_loss is None or trailing_stop is None:
        return False

    # fixed stop loss
    if current_price <= stop_loss:
        return True

    # trailing stop
    if current_price <= trailing_stop:
        return True

    return False

